...
import ClearpayTable from "./ClearpayTable";
import type { Metadata } from "next";

export const dynamic = "force-dynamic";

export const metadata: Metadata = {
  title: "Buy Mounjaro with Clearpay UK 2026 | Compare Providers | WeightLossPricesUK",
  description: "Compare UK providers offering Mounjaro with Clearpay payment. See prices across all doses from 2.5mg to 15mg. Always confirm Clearpay availability directly with the provider.",
...
function loadClearpayProviders() {
...
export default function MounjaroClearpayPage() {
  const { providers, lastUpdated } = loadClearpayProviders();
      ...
      <section style={{ background: "linear-gradient(160deg, #0f1f3d 0%, #1a3260 60%, #1e4d8c 100%)", color: "white", padding: "48px 0 56px", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ maxWidth: 1140, margin: "0 auto", padding: "0 24px", position: "relative" }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "rgba(14,159,138,0.18)", border: "1px solid rgba(14,159,138,0.35)", color: "#7de8d8", fontSize: "0.75rem", fontWeight: 600, letterSpacing: "0.06em", textTransform: "uppercase", padding: "4px 12px", borderRadius: 20, marginBottom: 16 }}>
            Mounjaro - Clearpay payment
          </div>
          <h1 style={{ fontFamily: SERIF, fontSize: "clamp(1.6rem, 3.5vw, 2.4rem)", fontWeight: 400, lineHeight: 1.2, marginBottom: 12, maxWidth: 560 }}>
            Buy Mounjaro with <span style={{ color: "#7de8d8" }}>Clearpay</span> in the UK
          </h1>
          <p style={{ color: "rgba(255,255,255,0.65)", fontSize: "0.95rem", lineHeight: 1.6, maxWidth: 520, margin: 0 }}>
            {providers.length} UK-registered providers list Clearpay as a payment option for Mounjaro (tirzepatide). Compare prices across all doses below. Clearpay availability must always be confirmed directly with the provider before ordering.
          </p>
        </div>
      </section>

      <ClearpayTable providers={providers} lastUpdated={lastUpdated} />
