...
const FAQS = [
  { q: "Which UK providers offer Mounjaro with Klarna?", a: "Based on our research, providers including Cured Pharmacy, Swift Doctor, Ashcroft Pharmacy, The Care Pharmacy, UK Meds, Cloud Pharmacy, Oushk Pharmacy and The Family Chemist have listed Klarna as a payment option. Availability can change at any time - always confirm directly on the provider's website before ordering." },
  { q: "Is Klarna available for all Mounjaro doses?", a: "Klarna availability can vary depending on the dose and the total order value. Some providers may only offer Klarna above a certain spend threshold. Always check the provider's checkout directly to confirm whether Klarna is available for your specific dose." },
  { q: "Can I use Klarna Pay in 3 for Mounjaro?", a: "Some providers support Klarna Pay in 3, while others may offer Klarna Pay Later. The specific Klarna products available depend on the individual provider and your Klarna account status. Check the provider's payment page for details." },
  { q: "Are there other buy-now-pay-later options for Mounjaro?", a: "Yes - some providers also accept PayPal Pay in 3 and Clearpay. You can filter by payment option on our main Mounjaro comparison page to find what suits you." },
];

export default function KlarnaTable({ providers, lastUpdated }: { providers: ProviderWithLogo[]; lastUpdated: string }) {
    ...
    <>
      ...
      <main style={{ width: "100%", maxWidth: 1140, margin: "0 auto", padding: "28px 24px 80px", boxSizing: "border-box" }}>
        ...
        <div style={{ background: "#fff", borderLeft: `4px solid ${TEAL}`, padding: "14px 20px", marginBottom: 24, display: "flex", alignItems: "flex-start", gap: 12, fontSize: "0.82rem", color: MUTED, lineHeight: 1.6 }}>
          ...
          <span>Klarna availability shown here is based on publicly available information and may change at any time. Always visit the provider&apos;s website directly to confirm whether Klarna is currently accepted and whether you are eligible before placing an order.</span>
        </div>

        {/* Table meta */}
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12, flexWrap: "wrap", gap: 8 }}>
          <span style={{ fontSize: "0.82rem", color: MUTED }}><strong style={{ color: NAVY }}>{rows.length}</strong> providers offering Klarna</span>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10, flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ minWidth: 0 }}>
                    ...
                    <div style={{ display: "flex", gap: 4, marginTop: 4, flexWrap: "wrap", alignItems: "center" }}>
                      ...
                      <span style={{ fontSize: "0.62rem", fontWeight: 700, padding: "1px 6px", borderRadius: 10, background: "#f5e6ff", color: "#7c3aed" }}>Klarna</span>
        ...
        <div style={{ background: "#fff", borderLeft: `4px solid ${TEAL}`, padding: "14px 20px", marginTop: 28, display: "flex", alignItems: "flex-start", gap: 12, fontSize: "0.8rem", color: MUTED, lineHeight: 1.55 }}>
          ...
          <span>Prices are for informational purposes only and may change at any time. Klarna availability is not guaranteed - always confirm directly with the provider before ordering. This is not medical advice - always consult a qualified healthcare professional before starting any medication.</span>
        </div>
        ...
        <div style={{ marginTop: 40, display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 16 }}>
          <div style={{ background: "#fff", border: `1.5px solid ${BORDER}`, borderRadius: 14, padding: 24, boxShadow: "0 1px 3px rgba(15,31,61,0.07)" }}>
            <h3 style={{ fontFamily: SERIF, fontSize: "1.1rem", fontWeight: 400, color: NAVY, marginBottom: 10, marginTop: 0 }}>Checking Klarna eligibility</h3>
            <p style={{ fontSize: "0.85rem", color: MUTED, lineHeight: 1.7, margin: 0 }}>Klarna availability can vary by provider, dose, and order value. It may also depend on your personal Klarna account status. Always check the provider&apos;s checkout process directly to confirm Klarna is available for your specific order before committing.</p>
          </div>
          <div style={{ background: "#fff", border: `1.5px solid ${BORDER}`, borderRadius: 14, padding: 24, boxShadow: "0 1px 3px rgba(15,31,61,0.07)" }}>
            ...
            <p style={{ fontSize: "0.85rem", color: MUTED, lineHeight: 1.7, margin: 0 }}>This page only shows providers that list Klarna as a payment option. To compare all UK Mounjaro providers - including those offering PayPal, subscription discounts, or standard card payment - visit the full comparison page.</p>
            ...
            {FAQS.map((faq, i) => (
              <div key={i} style={{ background: "#fff", border: `1.5px solid ${BORDER}`, borderRadius: 8, overflow: "hidden", boxShadow: "0 1px 3px rgba(15,31,61,0.07)" }}>
                <button onClick={() => setOpenFaq(openFaq === i ? null : i)} style={{ width: "100%", background: "none", border: "none", cursor: "pointer", padding: "18px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, textAlign: "left", fontFamily: "inherit", fontSize: "0.92rem", fontWeight: 600, color: NAVY }}>
                  {faq.q}
                  <span style={{ width: 22, height: 22, borderRadius: "50%", background: openFaq === i ? "#e6f7f5" : "#eef0f6", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, transition: "transform 0.22s, background 0.18s", transform: openFaq === i ? "rotate(180deg)" : "none", color: openFaq === i ? TEAL : MUTED, fontSize: "0.75rem" }}>▾</span>
                </button>
                {openFaq === i && (
